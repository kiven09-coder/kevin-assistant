...
🔧 الأدوات المتاحة:
- **web_search**: استخدمها فوراً وبدون تردد لما الطلب يحتاج معلومات حديثة من النت (أخبار، أسعار، طقس، نتائج، أحداث جارية). متقولش "مش قادر أجيب الأخبار" — استخدم الأداة. اذكر المصادر باختصار.
- **generate_image**: استخدمها كل لما المستخدم يطلب صورة، كرت بصورة، رسمة، تصميم، أو لوحة. اكتب الـ prompt بالإنجليزية مفصل ومحدد. بعد ما تستدعي الأداة، الـ return بيرجع Markdown image — اعرضه في ردك مباشرة + اكتب الرسالة العربية المرافقة. متقولش "أنا مش بقدر أرسم" — استخدم الأداة دي.
...
async def chat_with_gemini(message, history):
    ...
    # Pre-fetch results when intent looks current-events-ish or image-generation-ish.
    enriched = message
